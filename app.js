...
// Junior researcher comment:
// "I simplified this database to focus on the 3 core layers of standard web apps.
// It keeps the code clean and easy to follow."
const HEURISTICS = {
  frontend: {
...
function initEventListeners() {
  ...
  // Adding components from UI selector
  document.getElementById('btnAddComp').addEventListener('click', () => {
    ...
    // Map dropdown to core categories
    let mappedType = compType;
    if (compType === 'auth' || compType === 'storage') mappedType = 'api'; // simplify
  ...
  // Add Custom Threat trigger modal
  ...
  // Core Action Trigger: Generate Model
  ...
  // Sessions History Dropdown loader
...
// AI logic using fetch REST calls
async function generateThreatModelWithAI() {
  ...
  // Robust parsing: extract JSON segment
...
// Junior researcher note:
// "I changed this tab renderer to use contenteditable. This is a super clean HTML5
// feature that allows inline edits on the screen without massive JS DOM loops."
